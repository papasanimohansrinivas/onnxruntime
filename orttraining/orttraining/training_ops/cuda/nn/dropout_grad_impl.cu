...
// a different approach
  ...
  //   T dYvalue[NumElementsPerThread];
  //   bool maskvalue[NumElementsPerThread];

  //   CUDA_LONG id = start;
  // #pragma unroll
  //   for (int i = 0; i < NumElementsPerThread; i++) {
  //     if (id < N) {
  //       dYvalue[i] = dY_data[id];
  //       maskvalue[i] = mask_data[id];
  ...
  //   for (int i = 0; i < NumElementsPerThread; i++) {
  //     if (id < N) {
  //       dX_data[id] = T(float(dYvalue[i]) * maskvalue[i] * scale);
